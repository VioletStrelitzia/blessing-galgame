...
运行时用 `_execution_stack` 记录每一层条件结构是否已有分支执行过；该栈不入档（读档重放时自然重建）。条件右值支持数字或变量名，等值比较为浮点近似。运行期带防御：ELSE_IF/ELSE/END_IF 遇空栈报错并顺序继续（不查跳转表，防畸形表死循环），跳转表缺项兜底时告警留痕——两条路径编译期已保证不可达，仅作产物损毁时的护栏。
