...
当前实现已经有 `if`、`elif`、`else` 的运行时处理（缩进块语法，编译期配对校验并拍平）。`StoryManager.load_script()` 后会构建 `_jump_table`，用于在条件不满足时跳到下一个分支或结构末尾。

运行时用 `_execution_stack` 记录每一层条件结构是否已有分支执行过；该栈不入档（读档重放时自然重建）。条件右值支持数字或变量名，等值比较为浮点近似。
