...
管理状态 `Synchronizer.Mode`（模式机收编于同步器 Autoload）：

- `INTERACT`：玩家点击或按键推进。
- `AUTO`：对话完成后等待 `Global.auto_wait_time` 自动推进（语音在播则等语音播完再计时）。
- `SKIP`：跳过打字和等待，快速推进。
- `STOP`：屏蔽外部推进输入（为挂载玩法预留）。
...
执行模型为「同步主循环 + 同步器门闸」：`run_script` 不挂起协程，执行到对白/挂起/换幕即返回——**回合停止是结构判停**（对白展示 / `Synchronizer` 存在阻塞挂起 / 换幕中），不经 handler 返回值传播。一切挂起（等待计时、立绘动画、转场、选项、AUTO 等语音）都以凭证形式登记进 `Synchronizer` 的挂起注册表；完成事件（信号/定时器）释放凭证，`holds_cleared` 驱动续跑。抢占按优先级数路由处置：点击打断（FAST_FORWARD 直达终态 / KILL / RELEASE_ASYNC 按类别注册表）、进入 SKIP 清场、跳幕/读档全量复位。
...
选项由 `*` 选项行 + 缩进分支体组成（编译期拍平为 `OPTION`…`OPTION_END` 线性序列）。选项组结构在编译期回填进各 `OPTION` 参数（`is_head`/`body_start`/`next_option`/`group_end`）：组头到达时沿 `next_option` 链收集可见项（带 `if:` 条件的选项求值过滤，全假则跳过整组），然后挂载 `OptionUI` 等待玩家选择；零运行时扫描。

玩家选择后：

- `OptionUI` 发出 `option_made(index)`。
- UI 自行卸载并回到 `SceneManager` 的池中。
- `StoryManager` 把 `idx` 改到对应分支入口（回填的 `body_start`）继续执行。

分支体内若遇到兄弟 `OPTION`，按回填的 `group_end` 直跳组尾，避免误入其他分支。载入存档或退出时若当前有选项挂起，相关代码会发出 `canceled` 释放凭证（不续跑）。
...
- `>` 生成 `PostInstruction`。
- 指令使用 CLI 风格分词，支持双引号。
- 普通行按「首个不在 `[...]` 内的冒号」拆分说话人和内容（锚点修饰键里的冒号不算分隔符）。
- 没有此类冒号的行作为旁白。

指令经 `parse_instruction_line` 按 `match` 分派解析；参数形状由 `Instruction._SPEC` 表统一定义——每项 `[参数名, 类型, 默认值, 语义角色]`，提供默认值补齐、类型校验与 `from_strings` 转型。语义角色（role）为开放 `StringName`（如 `"audio"`/`"texture"`/`"script"`/`"res_path"`/`"scene_type"`）：编译期校验（`_validate_param_roles`，登记表对照/路径存在性/域检查）与工具链 schema 导出共用同一标注源——新增引用类参数只需一处标注，不写死、零新校验代码。
...
- `StoryManager.idx`
- `StoryManager._choice_log`（选项选择序列：各选项组选定分支的 body_start，按经过顺序；存档 v3 起）
- `Global.rng_seed`（随机种子，确定性重放用）
- 时间戳、标题和缩略图路径

载入时由 SaveUI 触发 `SaveManager._on_load_pressed()`，再调用 `StoryManager.load_game(saved_game)`。恢复过程并不是简单设置画面，而是：

1. 重置当前选项与挂起状态（`Synchronizer.reset()`）。
2. 恢复变量与随机种子（条件执行栈不入档——重放经过 IF/ELIF/ELSE 时自然重建）。
3. 设置 `next_script` 为存档中的脚本名。
4. `await next_story(false)` 重新挂载场景并加载剧本，但先不进入普通推进。
5. 设置 `_replay_end = sg.idx`，换入存档的 `choice_log`。
6. 切到 `SKIP` 模式快速重放到存档索引；重放经过选项组时按 `choice_log` 自动选定（不弹 UI、不等玩家），保证确定性重放。
7. 到点后 `choice_log` 截掉存档点之后的陈旧未来（玩家此后可能走不同分支），交还玩家。
