...
- 存档：保存当前脚本名、执行索引、变量和条件执行栈；载入后通过 SKIP 快速推进到存档点。
...
- `INITIALIZATION`：切入剧本时只执行前序指令，直到遇到第一条非前序事件。
- `DEFAULT`：正常执行前序指令、普通指令和对话。
- `POST_COMMAND`：对话后只连续执行后序指令，遇到非后序事件后回到默认流程。
...
1. `next_story()` 重置当前状态，淡出，卸载旧 UI/世界，清资源缓存。
2. 挂载 `对话UI`、`GalUI` 和 `Gal2D`。
3. `load_script(next_script)` 从 `ResourceManager` 加载 `.tres` 剧本。
4. `run_script(INITIALIZATION)` 执行开场前置指令，例如背景、音乐、初始立绘。
5. 淡入后 `run_script(DEFAULT)` 显示第一句对话并等待交互。
6. 玩家输入由 `DialogueUI.forward` 或 `fast_forward` 信号回到 `StoryManager`。

这个流程充分利用了 Godot 的 `await` 和信号机制：显示对话、等待选项、等待 Tween、等待转场都可以自然地写成协程，而不是手动拆成大量帧状态。
...
角色动画不是直接在 `StoryManager` 中执行 Tween，而是先把 `char setup/show/hide/move/wait/texture` 转为角色的 `animation_list`，再由 `char play` 触发播放。这让剧本指令可以先描述一组动画，再在合适时机统一执行。

## 选项和分支

选项由 `option` 和 `option end` 组成。`StoryManager._option_begin()` 第一次遇到选项时会向后扫描到 `OPTION_END`，记录每个选项文本和对应分支起点，然后挂载 `OptionUI` 等待玩家选择。
...
当前实现已经有 `if`、`elif`、`else`、`endif` 的运行时处理。`StoryManager.load_script()` 后会构建 `_jump_table`，用于在条件不满足时跳到下一个分支或结构末尾。

运行时用 `_execution_stack` 记录每一层条件结构是否已有分支执行过。存档会保存这个栈，因此在条件结构中读档时可以恢复对应执行上下文。
...
- 使用多个播放器组成池。
- 每次播放切到下一个播放器，支持短音效重叠。

Voice 管理器：

- 使用 `AudioEventManager`。
- 播放前可以注册时间事件。
- `voice event` 指令会把后续指令绑定到语音播放时间点。

设置界面通过 `AudioManager.set_volume()` 修改 Godot `AudioServer` 对应总线音量，实现即时生效。
